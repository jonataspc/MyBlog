...
namespace MyBlog.Core.Data.Helpers
{
    internal static class DbMigrationHelpers
    {
        ...
        {
            if (await context.Authors.CountAsync() > 2)
            {
                return;
            }
        ...
        private static async Task SeedAspNetIdentity(MyBlogDbContext context, IServiceProvider serviceProvider)
        {
            ...
            await AddOrdinaryUser(userManager);
            await context.SaveChangesAsync();
        }

        private static async Task AddOrdinaryUser(UserManager<ApplicationUser> userManager)
        {
        ...
        private static async Task AddAdminUser(UserManager<ApplicationUser> userManager)
        {
